...
static const struct proc_ops lct_proc_tp_grip_area_ops = {
...
int init_lct_tp_grip_area(set_screen_angle_callback set_fun, get_screen_angle_callback get_fun)
{
	...
	TP_LOGW("Initializing /proc/%s node!\n", TP_GRIP_AREA_NAME);
	...
	lct_tp_p->proc_entry_tp_grip_area = proc_create_data(TP_GRIP_AREA_NAME, 0666, NULL, &lct_proc_tp_grip_area_ops, NULL);
	if (!lct_tp_p->proc_entry_tp_grip_area) {
		TP_LOGE("Failed to add /proc/%s\n", TP_GRIP_AREA_NAME);
		kfree(lct_tp_p);
		return -EPERM;
	}

	TP_LOGW("/proc/%s created successfully!\n", TP_GRIP_AREA_NAME);
	return 0;
}
