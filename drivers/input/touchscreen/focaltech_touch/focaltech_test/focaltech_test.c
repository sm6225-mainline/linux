...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    written = kernel_write(pfile, data_buf, len, &pos);
    if (written < 0) {
        FTS_TEST_ERROR("error occurred while writing to file %s.", filepath);
    }
                        ...
                        bool mc_sc, bool key, bool result)
{
    ...
    info->name[TEST_ITEM_NAME_MAX - 1] = '\0';  // Явно завершаем строку
    info->code = code;
    info->mc_sc = mc_sc;
...
static int fts_tp_data_dump_proc_init(void)
{
    ...
    // Используем proc_ops вместо file_operations
