...
static const struct proc_ops lct_proc_tp_info_pops = {
	.proc_read		 = lct_proc_tp_info_read,
};

static const struct proc_ops lct_proc_tp_lockdown_info_pops = {
...
int init_lct_tp_info(char *tp_info_buf, char *tp_lockdown_info_buf)
{
	...
	if (tp_lockdown_info_buf)
		strcpy(lct_tp_p->tp_lockdown_info_buf, tp_lockdown_info_buf);

	lct_tp_p->proc_entry_tp_info = proc_create_data(TP_INFO_NAME, 0444, NULL, &lct_proc_tp_info_pops, NULL);
	if (IS_ERR_OR_NULL(lct_tp_p->proc_entry_tp_info)) {
		TP_LOGE("add /proc/%s error\n", TP_INFO_NAME);
		goto err_tp_info;
	}
	lct_tp_p->proc_entry_tp_lockdown_info = proc_create_data(TP_LOCKDOWN_INFO_NAME, 0444, NULL, &lct_proc_tp_lockdown_info_pops, NULL);
...
static ssize_t lct_proc_tp_info_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (*ppos) // Если позиция не нулевая, то конец файла
		return 0;

	if (lct_tp_p->pfun_info_cb)
		lct_tp_p->pfun_info_cb(TP_CALLBACK_CMD_INFO);

	// Определяем содержимое для вывода
	info_buf = (lct_tp_p->tp_info_buf && strlen(lct_tp_p->tp_info_buf))
			? lct_tp_p->tp_info_buf
			: "Unknown touchpad\n";

	cnt = strlen(info_buf);

	// Копируем данные в пользовательский буфер
	if (copy_to_user(buf, info_buf, cnt))
		return -EFAULT;

	*ppos += cnt; // Обновляем позицию
	return cnt;
}

static ssize_t lct_proc_tp_lockdown_info_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	const char *lockdown_buf;
	int cnt;
	...
	// Определяем содержимое для вывода
	lockdown_buf = (lct_tp_p->tp_lockdown_info_buf && strlen(lct_tp_p->tp_lockdown_info_buf))
			? lct_tp_p->tp_lockdown_info_buf
			: "Unknown touchpad\n";

	cnt = strlen(lockdown_buf);

	// Копируем данные в пользовательский буфер
	if (copy_to_user(buf, lockdown_buf, cnt))
		return -EFAULT;
